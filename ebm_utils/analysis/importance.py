import numpy as np



def get_feat_vals(ebm_global, feat_id):
    good_vals = ebm_global.data(feat_id)['names']
    return good_vals


def find_bin(val, ar):
    # Find the first bin which has left index geq the query.
    # The query belongs in the bin to the left of this one.
    # Assumes the bins are right-inclusive.
    try:
        return np.argmin(ar < val)
    except TypeError:
        return np.argmin([a == val for a in ar])
    raise NotImplementedError  # comparison between bins of objects that aren't exact is not supported


def calc_embeddings(ebm_global, X):
    preds = np.zeros((X.shape[0], len(ebm_global.feature_names)))
    for i in range(X.shape[0]):
        for j in range(X.shape[1]):
            good_vals = get_feat_vals(ebm_global, j)
            try:
                preds[i, j] = ebm_global.data(j)['scores'][find_bin(X[i, j], good_vals)]
            except KeyError:
                pass

            for k in range(j, X.shape[1]):  # j < k
                pair_feat_name = "{} x {}".format(ebm_global.feature_names[j], ebm_global.feature_names[k])
                if pair_feat_name not in ebm_global.feature_names:
                    continue
                else:
                    feat_idx = ebm_global.feature_names.index(pair_feat_name)
                    good_vals2 = get_feat_vals(ebm_global, k)
                    idx1 = find_bin(X[i, j], good_vals)
                    idx2 = find_bin(X[i, k], good_vals2)
                    preds[i, feat_idx] = ebm_global.data(feat_idx)['scores'][idx1, idx2]  # pairs[(j, k)][idx1, idx2]
    return preds


def calc_importance(ebm_global, X):
    embeddings = calc_embeddings(ebm_global, X)
    return np.mean(np.abs(embeddings), axis=0)
